---
name: handoff
description: Claude Design 핸드오프 패키지(zip/폴더) 하나로 iOS · Android · web · backend 를 한 번에 만드는 워크플로의 유일한 진입점. 패키지 등록 → 스펙·인프라 결정 → openapi → 계약 확정(사람) → 워크트리 구현(기본 직렬) → 서버 검사 → 완료 승인(사람) → 머지. 새로 시작하든 이어하든 여기서 시작한다.
---
...
| ⑤ build | `build` → 응답의 `dispatch` 가 착수 방식을 정한다. **기본은 `mode: "serial"`** — `dispatch.next` 역할 **하나만** 그 역할의 `prompts` 로 띄우고, 그 역할이 `report` 를 낸 뒤 `build` 를 다시 불러 다음 역할을 받는다. 동시에 둘을 띄우지 않는다 (한 기기에서 iOS·Android 툴체인이 코어를 나눠 쓰면 둘 다 느려지고, 한쪽이 막히면 세션 전체가 길어진다). `mode: "parallel"` 일 때만 `dispatch.order` 전부를 한 번에 띄운다 — `.handoff/config.json` 의 `dispatch.mode` 로 바꾼다. 프롬프트는 영어다 — 번역·요약하지 말고 그대로 전달하고, 진행 중 관여하지 않는다. |
